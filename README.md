...
**没人开着页面的时候也在积累趋势数据**。打开页面时，如果 `status.json` 已经比
存档旧了，面板会顺手再跑一次（见下）。
...
- **面板是单个 HTML 文件**，没有构建步骤、没有外部 JS 依赖。要改样式直接改
  `web/index.html`。
- **解析永远发生在子进程里**。解压 + GVAS 解析峰值约 450MB，其中 170MB 左右
  Python 不会还给系统——常驻进程干这件事，几分钟就胖到 200MB+ 并且不再瘦下来。
  交给一个跑完就退的子进程，常驻的 web 服务稳定在 20MB。
- **同一份存档不重复解析**。服务器每隔几分钟才写一次 `Level.sav`，中间再解析
  一次得到的是一模一样的数字。所以 `status.json` 比存档新时，请求直接返回文件
  （亚毫秒），只有存档真的变了、或者超过 `live_max_age_seconds` 才重算。
